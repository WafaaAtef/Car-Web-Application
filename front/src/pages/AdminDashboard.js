...
`
  * { box-sizing: border-box; }
  ...
  .auth-spinner {
    ...
    border: 2px solid rgba(196,164,96,0.15);
    border-top-color: #c4a460;
    border-radius: 50%;
  ...
  .btn-add {
    ...
    background: #c4a460; color: #080809; border: none;
    padding: 12px 28px; border-radius: 2px; cursor: pointer;
    transition: all 0.2s ease; display: flex; align-items: center; gap: 8px;
  }
  .btn-add:hover { background: #d4b470; transform: translateY(-1px); }
  .btn-add:active { transform: translateY(0); }

  .admin-body { padding: 40px 48px; }

  .admin-stats {
    display: grid; grid-template-columns: repeat(4, 1fr);
    gap: 1px; margin-bottom: 40px;
    background: rgba(255,255,255,0.05); border-radius: 2px; overflow: hidden;
  }
  .admin-stat { background: #0d0d0f; padding: 20px 24px; }
  .admin-stat-label { font-size: 9px; font-weight: 600; letter-spacing: 3px; text-transform: uppercase; color: rgba(255,255,255,0.25); margin-bottom: 6px; }
  .admin-stat-value { font-family: 'Barlow Condensed', sans-serif; font-size: 28px; font-weight: 700; color: #c4a460; }
  ...
  .empty-sub { margin-top: 12px; font-size: 13px; color: rgba(255,255,255,0.15); }
  .btn-empty-add {
    margin-top: 24px; font-family: 'Barlow Condensed', sans-serif;
    font-size: 12px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase;
    background: transparent; color: #c4a460; border: 1px solid rgba(196,164,96,0.3);
    padding: 10px 24px; border-radius: 2px; cursor: pointer; transition: all 0.2s; display: inline-block;
  }
  .btn-empty-add:hover { background: rgba(196,164,96,0.07); }

  .request-card {
    background: #0d0d0f; padding: 20px; border-radius: 4px;
    min-width: 280px; flex: 1 1 320px; border: 1px solid rgba(255,255,255,0.08);
  }
  .req-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; }
  .req-id { font-size: 14px; color: #c4a460; font-weight: 700; }
...
function AdminDashboard() {
  ...
  const handleDelete = async (id) => {
    ...
    await fetch(`/api/cars/${id}`, { method: "DELETE", credentials: "include" });
    fetchCars();
  };

  const handleEdit = (car) => navigate(`/edit/${car._id}`, { state: car });
  ...
  const handleApprove = async (requestId) => {
    ...
    if (res.ok) { fetchRequests(); fetchCars(); }
  };
      ...
      <div className="admin-dash">
        ...
        <div className="admin-body">
          ...
          <div className="cars-grid">
            {cars.length > 0 ? (
              cars.map(car => (
                <CarCard key={car._id} car={car} isAdmin={true} onDelete={handleDelete} onEdit={handleEdit} />
              ))
            ) : (
              <div className="empty-state">
                <div className="empty-text">No vehicles yet</div>
                <button className="btn-empty-add" onClick={() => navigate("/add")}>+ Add First Vehicle</button>
              </div>
            )}
          </div>
          ...
          <div className="cars-grid" style={{ gap: "18px" }}>
            {requests.length > 0 ? (
              requests.map((request) => (
                <div key={request._id} className="request-card">
                  <div className="req-header">
                    <div className="req-id">#{request._id.slice(-6)}</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: request.status === "confirmed" ? "#6fcf97" : request.status === "cancelled" ? "#eb5757" : "#ffd166" }}>
                      {request.status.toUpperCase()}
                    </div>
                  </div>
                  <div style={{ color: "#fff", fontWeight: 700, marginBottom: 6 }}>{request.car?.brand} {request.car?.model}</div>
                  <div style={{ fontSize: 12, color: "#ddd", marginBottom: 4 }}>Buyer: {request.buyer?.firstname || "Unknown"} {request.buyer?.lastname || ""}</div>
                  <div style={{ fontSize: 12, color: "#ddd", marginBottom: 4 }}>Price: {Number(request.price).toLocaleString("en-EG")} EGP</div>
                  <div style={{ fontSize: 11, color: "#999", marginBottom: 12 }}>Requested on: {new Date(request.createdAt).toLocaleDateString()}</div>
                  <div className="req-actions">
                    {request.status === "pending" ? (
                      <>
                        <button className="btn-approve" onClick={() => handleApprove(request._id)}>Approve</button>
                        <button className="btn-reject" onClick={() => handleCancel(request._id)}>Cancel</button>
                      </>
                    ) : (
                      <div style={{ width: "100%", padding: "10px 12px", background: "#141414", borderRadius: 2, color: "#aaa" }}>No actions available</div>
                    )}
                  </div>
                </div>
              ))
              ...
              <div className="empty-state" style={{ padding: "40px" }}>
                ...
                <div className="empty-sub">Requests will appear here once submitted.</div>
              </div>
            )}
          </div>
        </div>
